...
class Kiosk extends React.Component {
    ...
    confirmSend = (value) => {
        if (value) {
            axios.post("http://192.168.11.190:5000/customers", { msisdn: this.state.phoneNumber, queueId: this.state.clickedQueueId, queueName: this.state.queueName })
            .then((res) => {
                console.log(res);
            })
            .catch((err) => {
    ...
    componentDidMount() {
        axios.get("http://192.168.11.190:5000/queues")
            .then((res) => {
